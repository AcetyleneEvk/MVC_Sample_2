...
namespace BlogSample.Controllers
{
    public class ZipCodeController : Controller
    {
        ...
        private SOTWOEntities1 source_For = new SOTWOEntities1();
        public ActionResult Index(int page = 1)
        {
            int currentPage = page < 1 ? 1 : page;

            //var quString = source_For.Store_Base
            //     .Where(x => x.ID == 1 )
            //     .OrderBy(x => x.ID);

            //var result = quString.ToPagedList(currentPage, 10);

            var query = tEST.TEST_Table
                           .Where(x => x.userName == "evk" && x.userPassword == "") 
                           .OrderBy(x => x.userName)
                           .ThenBy(x => x.ID)
                           .ThenBy(x => x.userPassword);

           var JAD =  query.ToList();
        ...
        {
            JObject jo = new JObject();
            bool result = !tEST.TEST_Table.Count().Equals(0);
        ...
        private JArray GetExportData()
        {
            ...
            foreach (var item in query)
            {
                var jo = new JObject();
                jo.Add("ID", item.ID);
                jo.Add("userName", item.userName);
                jo.Add("userPasswrod", item.userPassword);
                jObjects.Add(jo);
            }
            return jObjects;
        }
